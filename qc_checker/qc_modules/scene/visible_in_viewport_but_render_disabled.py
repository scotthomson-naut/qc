...
# Meta data
SEVERITY = "warning"
LABEL = "Visible In Viewport But Render Disabled"
DESCRIPTION = (
    "Checks if renderable objects are visible in the viewport (Eye "
    "ON, Monitor ON) but disabled from render (Camera OFF). Skips "
    "control-type objects (Empty/Armature/Lattice, CTRL_/REF_ naming, "
    "or rig_controls/guides collections), since this is their normal "
    "designed state. Also skips objects in hero/deliverable "
    "collections - those are covered by a separate, Critical-severity "
    "check instead."
)
...
def main():
    ...
    """
    failed_objects = get_viewport_visible_render_disabled_objects()

    issues = [
        "Failed object: {} - Visible in viewport but disabled from "
...
#
# This flag combination is a legitimate, common pattern for reference
# or proxy geometry kept visible for layout purposes but intentionally
# excluded from render - it isn't safe to assume it's a mistake and
# auto-toggle it. Needs an artist to confirm intent.
...
# -------------------------
#
# Duplicated in visibility_cascade_critical_deliverable.py (module 2b)
# by design choice - this framework doesn't currently share code
# between check modules, so if these rules ever change (e.g. adding a
# new control-name prefix), both files need to be updated by hand.
...
def get_viewport_visible_render_disabled_objects(
        objects=None,
        view_layers=None,
    ):
    """
    Finds objects visible in the viewport but disabled from render,
    excluding control-type objects and objects in hero/deliverable
    collections.
    ...
    for view_layer in view_layers:
        for obj in objects:
            ...
            if is_object_in_deliverable_collection(obj):
                # Covered by the separate Critical-severity check
