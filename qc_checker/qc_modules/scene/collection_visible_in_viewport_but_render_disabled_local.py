...
# Meta data
SEVERITY = "info"
LABEL = "Collection Visible In Viewport But Render Disabled (Local)"
DESCRIPTION = (
    "Checks if collections are visible in the viewport via the per-"
    "view-layer Eye icon (LayerCollection.hide_viewport is False) but "
    "disabled from render (Camera OFF, global). Scoped to one view "
...
def main():
    ...
    """
    failed_objects = get_eye_visible_render_disabled_collections()

    issues = []

    for collection_name, collection_data in failed_objects.items():
        issues.append(
            "Failed collection: {} - Visible via Eye icon but "
            "disabled from render, in view layer(s): {}".format(
                collection_name,
...
#
# Toggling a collection's Eye or Camera icon affects every object
# inside it at once - not safe to assume it's a mistake and
# auto-toggle it for the whole collection.
...
def get_eye_visible_render_disabled_collections(view_layers=None):
    """
    Finds collections visible via the per-view-layer Eye icon
    (LayerCollection.hide_viewport is False) that are disabled from
    render (Collection.hide_render is True).
    ...
    Note:
        ...
        visibility_collection_visible_render_disabled_global.py,
    ...
    for view_layer in view_layers:
        for layer_collection in iterate_layer_collections(
            ...
            if not eye_hidden and collection.hide_render:
